...
class CompleteReadData {
  ...
  function __construct($token) {
    try {
      // Step 0 - initialize.
      // Initialize api.
      Semstorm::init( $token, 'http://qazwsx.semstorm.com:91/api-v3/' );
      ...
      // New filters object for easy usage of filters.
      $this -> filters = new SemstormFilters();
    } catch ( Exception $e ) {
  ...
  function getCampaignKeywords( $campaignId ){
    try {
      ...
      $keywords = [];
      $results = $this -> monitoringKeyword -> getList();
      while(!empty($results['keywords'])){
        $keywords = array_merge($keywords, $results['keywords']);
        $params['pager']['page']++;
      }
  ...
  function getKeywordDetailsFromJuly($id, $type) {
    try {
      ...
      $params['params']['datemin'] = "20170701";
      $params['params']['datemax'] = "20170731";
      $params['params']['gap'] = "daily";
      // Set data type.
  ...
  function getCampaignMontlyDataFromFirstHalfOfYear($id) {
    try {
      ...
      $params['params'] = [ ];
      $params['params']['datemin'] = "20170101";
      $params['params']['datemax'] = "20170630";
  ...
  function getCampaignHighVolumeData($id) {
    try {
      ...
      $params['params']['datemin'] = "20170701";
      $params['params']['datemax'] = "20170737";
      $params['params']['gap'] = "daily";
      // Add filter "Only keywords with search volume more than 6000".
      $this -> filters -> addFilter( 'volume', 6000, 'more' );
      // Put filters into parameters.
      $params['params']['filters'] = $this -> filters -> getFilters();
      // Reset filters (for another use).
